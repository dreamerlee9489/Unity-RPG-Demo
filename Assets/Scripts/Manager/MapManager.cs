...
namespace Manager
{
    public class MapManager : MonoBehaviour
    {
        ...
        public string accessPath
        {
            get
            {
                return InventoryManager.Instance.playerData.nickName + "_MapData_" + SceneManager.GetActiveScene().name;
            }
            private set { }
        }
        ...
        void Awake()
        {
            enemies = GameObject.FindGameObjectsWithTag("Enemy").Select(enemy => enemy.GetComponent<Entity>())
            ...
            {
                Item item = Instantiate(Resources.Load<Item>(mapData.mapItemDatas[i].path),
                    new Vector3(mapData.mapItemDatas[i].position.x, mapData.mapItemDatas[i].position.y,
                        mapData.mapItemDatas[i].position.z), Quaternion.Euler(90, 90, 90));
